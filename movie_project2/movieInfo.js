...
window.onload = () => {
  ...
  const movieId = parseInt(params.get("id")); // 문자열 → 숫자 변환
  ...
  if (movieId) {
    ...
    if (movie) {
      ...
      document.getElementById("release-date").textContent = `개봉일: ${movie.release_date}`;
      document.getElementById("vote").textContent = `평점: ${movie.vote_average}`;
      document.getElementById("poster").src = `https://image.tmdb.org/t/p/w500${movie.poster_path}`;
    } else {
      document.getElementById("title").textContent = "해당 영화 정보를 찾을 수 없습니다.";
    }
  } else {
    document.getElementById("title").textContent = "영화 ID가 제공되지 않았습니다.";
  }
